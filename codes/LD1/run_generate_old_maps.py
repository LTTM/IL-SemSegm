...
def test():
    ...
    with graph.as_default():
        ...
        mean_IoU, mean_IoU_update_op, G_output_maps, next_images, next_labels, validation_summary = gan.validation_model()
    ...
    with tf.Session(graph=graph, config=None) as sess:
        ...
        try:
            ...
            for i in range(FLAGS.validation_batch_size):
                try:
                    print(str(i))
                    output_maps, next_images_run, next_labels_run = sess.run([G_output_maps, next_images, next_labels])
