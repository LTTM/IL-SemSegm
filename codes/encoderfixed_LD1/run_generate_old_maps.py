...
def test():
    ...
    with tf.Session(graph=graph, config=None) as sess:
        ...
        try:
            ...
            #os.makedirs('softmax_output_mat/')
            #os.makedirs('next_images_mat/')
            #os.makedirs('next_labels_mat/')
            #os.makedirs('feature_maps_mat/')
        except os.error:
            pass
        ...
        try:
            ...
            for i in range(FLAGS.validation_batch_size):
                try:
                    ...
                    #print(next_images_run.shape)
                    #print(next_labels_run.shape)

                    np.save('softmax_output/'+str('{:04d}'.format(i))+'_softmax_output.npy', output_maps)
                    #tensor_utils.save_matlab_files('softmax_output_mat/', '{:04d}'.format(i), output_maps)

                    #np.save('feature_maps/' + str('{:04d}'.format(i)) + '_feature_maps.npy', features_maps)
                    #tensor_utils.save_matlab_files('feature_maps_mat/', '{:04d}'.format(i), features_maps)

                    np.save('next_images/' + str('{:04d}'.format(i)) + '_images.npy', next_images_run)
                    #tensor_utils.save_matlab_files('next_images_mat/', '{:04d}'.format(i), next_images_run)

                    np.save('next_labels/' + str('{:04d}'.format(i)) + '_labels.npy', next_labels_run)
                    #tensor_utils.save_matlab_files('next_labels_mat/', '{:04d}'.format(i), next_labels_run)
